...
import { HiFolder, HiChartSquareBar, HiUser, HiMenu, HiAdjustments, HiLogout } from 'react-icons/hi';
        ...
        <div className='mt-4 flex flex-col gap-3 relative'>
          {
            menus?.map((menu, index) => (
              <Link to={menu?.path} key={index} className={`group flex mx-2 rounded items-center text-sm gap-5 font-medium  hover:bg-gray-700 duration-200 ${isOpen ? 'p-3' : 'p-3'}`}>
                ...
                <h2 className={`${isOpen ? 'hidden' : 'visible'} absolute z-50 overflow-hidden left-32 bg-gray-100 font-semibold whitespace-pre text-gray-900 rounded drop-shadow-lg px-0 py-0 w-0 group-hover:px-2 group-hover:py-1 group-hover:left-20 group-hover:duration-200 group-hover:w-fit`}>
                  {menu?.name}
                </h2>
              </Link>
            ))
