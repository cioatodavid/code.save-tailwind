...
const Sidebar = () => {
  ...
  return (
    <section className='flex gap-6'>
      <div className={`bg-gray-800 pt-2 min-h-screen text-gray-50 ${isOpen ? 'w-44' : 'w-16'} duration-200`}>
        ...
        <div className='mt-4 flex flex-col gap-3 relative'>
          {
            menus?.map((menu, index) => (
              <Link to={menu?.path} key={index} className={`group flex mx-2 rounded items-center text-sm gap-5 font-medium  hover:bg-gray-700 hover:shadow-2xl  duration-150 ${isOpen ? 'p-3' : 'p-3'}`}>
